...
* Installation
  This package is on MELPA [[https://melpa.org/#/zetteldesk][file:https://melpa.org/packages/zetteldesk-badge.svg]] so just install it with ~M-x package-install~.

  The extensions ~zetteldesk-kb~ and ~zetteldesk-ref~ are currently also on MELPA and I am waiting for feedback on the rest. They should be there in at most a month. For everything not on MELPA, my current suggestion is to clone the repo and add the packages to the load path so you can load them to your config.
