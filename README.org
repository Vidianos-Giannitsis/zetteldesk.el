...
* Getting Started
  ...
  - Insert the contents of an org-file to the zetteldesk-scratch as supplementary material to org-roam nodes. Again, the selection is filtered to org files (*but not* org-roam nodes) in the zetteldesk. 
  - Insert links to specific pages of a pdf, with a selection menu of only zetteldesk pdfs
  - A special behaviour for inserting reference nodes (nodes which are associated with a bibtex entry) as they have very different formatting than your typical org-roam node.
  - Save specific parts of a manual inside the Info program to seamlessly jump between specific info nodes, even if unrelated. Furthermore, have the ability to insert the Info node to the zetteldesk-scratch.

    The last two are implemented in a seperate extension zetteldesk-ref.el, which I highly recommend.
...
* Related Work & Thank yous
  ...
  For the people that helped me, this was originally no one, because I made this myself on my free time. This has changed now however. @jlcolbert on github, has helped me with adding doom emacs related sections to the README/wiki, which is a nice contribution for me. @jdoggz helped me fix the graininess that the demonstrative gifs had in the beginning. The gifs were too large, but also grainy when viewed on firefox. He helped me get better quality gifs so its easier for everyone to follow.
